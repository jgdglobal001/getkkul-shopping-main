...
export const metadata: Metadata = {
  title: "?í’ˆ ì¹´í…Œê³ ë¦¬ | Getkkul-shopping",
  description:
    "?„ìž?œí’ˆ, ?¨ì…˜, ?ˆë°ì½? ë·°í‹° ???¤ì–‘???í’ˆ ì¹´í…Œê³ ë¦¬ë¥??˜ëŸ¬ë³´ì„¸?? ?í•˜???í’ˆ???½ê²Œ ì°¾ì„ ???ˆìŠµ?ˆë‹¤.",
  keywords: [
    "?í’ˆ ì¹´í…Œê³ ë¦¬",
    "?„ìž?œí’ˆ",
    "?¨ì…˜",
  ...
  openGraph: {
    title: "?í’ˆ ì¹´í…Œê³ ë¦¬ | Getkkul-shopping",
    description:
      "?¤ì–‘???í’ˆ ì¹´í…Œê³ ë¦¬ë¥??˜ëŸ¬ë³´ì„¸?? ?í•˜???í’ˆ???½ê²Œ ì°¾ì„ ???ˆìŠµ?ˆë‹¤.",
    url: "/categories",
...
export default async function CategoriesPage() {
  ...
  try {
    ...
    if (!categoriesResponse.ok) {
      console.error(`ì¹´í…Œê³ ë¦¬ API ?¤ë¥˜: ${categoriesResponse.status}`);
      categoriesData = [];
    } else {
      categoriesData = await categoriesResponse.json();
    }
  } catch (error) {
    console.error("ì¹´í…Œê³ ë¦¬ ?˜ì¹­ ?¤íŒ¨:", error);
    categoriesData = [];
  }
