...
const ProductsPage = async ({ searchParams }: Props) => {
  ...
  // ?îÎ? Ï∞∏Í≥†???ÅÌíà (Î™®Î∞î??Ïπ¥ÌÖåÍ≥†Î¶¨Îß?
  ...
  let products = [...dbProducts]; // DB ?ÅÌíà??Î©îÏù∏?ºÎ°ú
  ...
  // Ïπ¥ÌÖåÍ≥†Î¶¨ ?ÑÌÑ∞Îß?- smartphones???îÎ??êÏÑú Í∞Ä?∏Ïò§Í∏?
  if (params.category === "smartphones") {
    products = dummyProducts;
  } else if (params.category) {
    // ?§Î•∏ Ïπ¥ÌÖåÍ≥†Î¶¨??DB?êÏÑúÎß?
  ...
  // Filter by search term (DB?êÏÑúÎß?Í≤Ä??
  if (params.search) {
  ...
  const getPageTitle = () => {
    if (params.category) {
      switch (params.category) {
        case "bestsellers":
          return t("categories.bestsellers", "Î≤†Ïä§?∏Ï???);
        case "new":
          return t("categories.new_arrivals", "?†ÏÉÅ??);
        case "offers":
          return t("categories.special_offers", "?πÎ≥Ñ ?†Ïù∏");
        default:
          return `${
            params.category.charAt(0).toUpperCase() + params.category.slice(1)
          } ${t("products.product_header", "?ÅÌíà")}`;
      }
    }
    if (params.search) {
      const template = t("products.search_results_for", `"{{query}}" Í≤Ä??Í≤∞Í≥º`);
      return template.replace("{{query}}", params.search);
    }
    return t("products.all_products", "Î™®Îì† ?ÅÌíà");
  };
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 text-lg">
          {params.category || params.search
            ? (() => {
                const template = t("products.found_products", "{{count}}Í∞??ÅÌíà Ï∞æÏùå");
                return template.replace("{{count}}", String(products.length));
              })()
            : (() => {
                const template = t("products.discover_collection", "{{count}}Í∞??ÅÌíà???ÑÏ≤¥ Ïª¨Î†â??Î∞úÍ≤¨");
                return template.replace("{{count}}", String(products.length));
              })()}
        </p>
          ...
          <ol className="flex items-center space-x-2 text-gray-500">
            ...
            <li>
              <Link href="/products" className="hover:text-gray-700">
                {t("products.breadcrumb_products", "?ÅÌíà")}
              </Link>
            </li>
            {params.category && (
              <>
