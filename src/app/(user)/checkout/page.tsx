  ...
  const handleTossPayment = async () => {
    try {
      ...
      if (!paymentWidgetRef.current) {
        throw new Error("Í≤∞Ï†ú ?ÑÏ†Ø???ÑÏßÅ Ï§ÄÎπÑÎêòÏßÄ ?äÏïò?µÎãà?? ?†Ïãú ???§Ïãú ?úÎèÑ?¥Ï£º?∏Ïöî.");
      }

      // Validate order data
      if (!existingOrder) {
        throw new Error("Ï£ºÎ¨∏ ?ïÎ≥¥Î•?Ï∞æÏùÑ ???ÜÏäµ?àÎã§.");
      }
      ...
      const errorMessage = error instanceof Error ? error.message : "?§Ïãú ?úÎèÑ?¥Ï£º?∏Ïöî.";

      if (errorMessage.includes("Ï∑®ÏÜå")) {
        // User cancelled the payment - no alert needed
        console.log("Payment cancelled by user");
      } else {
        // Other errors - show alert
        alert(`Í≤∞Ï†ú ?§Ìå®: ${errorMessage}`);
      }
    } finally {
      setPaymentProcessing(false);
    }
