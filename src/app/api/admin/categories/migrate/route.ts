...
export async function POST(request: NextRequest) {
  try {
    // ë§ˆì´ê·¸ë ˆ?´ì…˜?€ ê´€ë¦¬ì ê¶Œí•œ ?†ì´ ?¤í–‰ ê°€??(??ë²ˆë§Œ ?¤í–‰?˜ë„ë¡?ë³´í˜¸??
    ...
    if (existingCount > 0) {
      return NextResponse.json(
        { error: "?´ë? ì¹´í…Œê³ ë¦¬ê°€ ì¡´ì¬?©ë‹ˆ?? ë§ˆì´ê·¸ë ˆ?´ì…˜???¤ì‹œ ?¤í–‰?????†ìŠµ?ˆë‹¤." },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      {
        message: `${createdCategories.length}ê°œì˜ ì¹´í…Œê³ ë¦¬ê°€ ë§ˆì´ê·¸ë ˆ?´ì…˜?˜ì—ˆ?µë‹ˆ??,
    ...
    console.error("ì¹´í…Œê³ ë¦¬ ë§ˆì´ê·¸ë ˆ?´ì…˜ ?¤ë¥˜:", error);
    const errorMessage = error instanceof Error ? error.message : "?????†ëŠ” ?¤ë¥˜";
    return NextResponse.json(
      { error: `ì¹´í…Œê³ ë¦¬ ë§ˆì´ê·¸ë ˆ?´ì…˜ ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤: ${errorMessage}` },
      { status: 500 }
    );
  }
}
